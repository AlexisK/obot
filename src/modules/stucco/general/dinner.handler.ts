...
import {dinnerFormatter} from './dinner.formatter';
...
export const dinnerHandler = new Handler({
  pattern : /(^|\W)(обе(д|т)|dinner)($|\W)/gi,
  mention(message : Message) {
    places.forEach(place => message.reply(dinnerFormatter.format(place)));
  }
});
